...
// POST /api/facturas/pagar — cliente paga su pedido
async function pagarPedido(req, res) {
  const { id_pedido, metodo_pago, propina } = req.body;
  const id_usuario = req.usuario.id_usuario;

  try {
    // Obtener id_cliente
    const [[cliente]] = await db.query(
      "SELECT id_cliente FROM clientes WHERE id_usuario = ?", [id_usuario]
    );
    if (!cliente) return res.status(404).json({ error: "Cliente no encontrado" });

    // Verificar que el pedido pertenece al cliente y está listo
    const [[pedido]] = await db.query(
      "SELECT id_pedido, estado, id_mesa, id_sede FROM pedidos WHERE id_pedido = ? AND id_cliente = ?",
      [id_pedido, cliente.id_cliente]
    );
    if (!pedido) return res.status(404).json({ error: "Pedido no encontrado" });
    if (pedido.estado !== "listo") return res.status(400).json({ error: "El pedido aún no está listo" });

    // Verificar que no tenga factura ya
    const [[facturaExiste]] = await db.query(
      "SELECT id_factura FROM facturas WHERE id_pedido = ?", [id_pedido]
    );
    if (facturaExiste) return res.status(409).json({ error: "Este pedido ya fue facturado" });

    // Obtener id_metodo_pago por nombre
    const [[metodo]] = await db.query(
      "SELECT id_metodo FROM metodos_pago WHERE nombre = ?", [metodo_pago]
    );
    if (!metodo) return res.status(400).json({ error: `Método de pago "${metodo_pago}" no encontrado` });
    ...
    // Insertar factura
    const [result] = await db.query(
      `INSERT INTO facturas (id_pedido, id_cliente, id_sede, id_metodo_pago, subtotal, iva_porcentaje, iva_valor, propina, total)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id_pedido, cliente.id_cliente, pedido.id_sede, metodo.id_metodo, subtotal, ivaPct, iva_valor, propina_v, total]
    );

    // Marcar como entregado y liberar mesa
    await db.query("UPDATE pedidos SET estado = 'entregado' WHERE id_pedido = ?", [id_pedido]);
    await db.query("UPDATE mesas SET disponible = 1 WHERE id_mesa = ?", [pedido.id_mesa]);
...
// GET /api/facturas/mis-facturas-cliente — facturas del cliente autenticado
async function getMisFacturasCliente(req, res) {
  const id_usuario = req.usuario.id_usuario;
  try {
    const [[cliente]] = await db.query(
...
module.exports = { getFacturas, getMisFacturas, crearFactura, pagarPedido, getMisFacturasCliente };
